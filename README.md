...
- **No subtitles, no scrubber, no automatic next-episode playback.** Continue watching (above) remembers *what* you were watching and, when it can, names what's next — it cannot resume *where* you left off, or start that next episode for you unasked, because nothing here reads back from mpv, iina, vlc, or a browser tab; none of them report a position. (When a remembered torrent holds several files, the picker does open on that next episode — one keypress, not none.)
