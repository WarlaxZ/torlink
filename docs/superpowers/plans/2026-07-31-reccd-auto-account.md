...
- **The foot-gun, verified against the live service — this one is not optional.** reccd's `POST /login` *reissues* the account's bearer token, replacing the old one. So a user who claims their account and then signs in at `reccd.stream` **invalidates the token torlink is holding**: the Accounts row starts reading `Token rejected` and recommendations stop. The feature's own pitch is "claim it so you can sign in elsewhere", and doing exactly that breaks the local install, so the README must say so plainly and say how to recover — press `↵` on the reccd row in Accounts and paste the token that `/login` returned. Do not bury this in a footnote; a user who hits it with no warning will read it as torlink being broken.
